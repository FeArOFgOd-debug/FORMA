...
async def run_pipeline(idea: str, *, job_id: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        phase1_tasks = [*exa_tasks, reddit_task, twitter_task]
        ...
        # --- Phase 2+3: sentiment + analysis in parallel ---
        _progress(job_id, 50, "Analyzing sentiment & generating report")

        sentiment_coro = None
        if reddit_posts or twitter_posts:
            sentiment_coro = _safe(
                "sentiment",
                _run_in_thread(analyze_sentiment, reddit_posts, twitter_posts, idea),
            )
        ...
        empty_sentiment: dict[str, Any] = {
            ...
            "summary": "No social media data was available for analysis.",
            "key_concerns": [],
            "key_positives": [],
            "notable_comments": [],
        }
        ...
        analysis_coro = _safe(
            "analysis",
            _run_in_thread(generate_analysis, idea, web_results, empty_sentiment),
        )

        if sentiment_coro:
            (_, sentiment_result, sentiment_err), (_, analysis_result, analysis_err) = await asyncio.gather(
                sentiment_coro, analysis_coro
            )
        else:
            sentiment_result = None
            sentiment_err = None
            _, analysis_result, analysis_err = await analysis_coro

        sentiment: dict[str, Any] = empty_sentiment
        if sentiment_err:
